...
## ⚠️ **重要: このドキュメントは検討資料です**

このドキュメントは、家族機能の設計検討段階で作成されたものです。
**最終的に採用された実装とは異なる内容が含まれています。**

### 📌 実装済みの正式な仕様書

- **[27_家族ひもづけ機能_管理ダッシュボード仕様書.md](27_家族ひもづけ機能_管理ダッシュボード仕様書.md)** ✅ **← こちらを参照してください**

### 🔍 このドキュメントとの主な違い

| 項目 | このドキュメント（検討資料） | 実装（27番） |
|------|---------------------------|-------------|
| **役割名** | `representative`/`member` | `parent`/`child` |
| **10倍整数システム** | 採用を検討 | **不採用**（整数のまま） |
| **0.1単位スタンプ** | スロットゲームで0.3〜0.8を付与 | **不採用**（整数のみ） |
| **`stamp_history.amount`** | 付与量（NUMERIC型、0.3〜0.8） | 付与量（INTEGER型、DEFAULT 1） |
| **スタンプ計算** | 10倍管理（10点=1スタンプ） | 1対1管理（1スタンプ=1） |

### 📚 このドキュメントの用途

- 設計検討時の議論・懸念点の記録（アーカイブ）
- 「なぜこの設計にしたのか」の背景理解
- 将来の拡張検討時の参考資料

**⚠️ 実装時は必ず27番のドキュメントを参照してください。**
